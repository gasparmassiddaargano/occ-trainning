...
export const ADBMLeftNavigation = () => import('../index');

describe('ADBMLeftNavigation renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ADBMLeftNavigation};
  beforeAll(async () => {
    /**
  ...
  it('ADBMLeftNavigation works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ADBMLeftNavigation',
            regions: []
          }
        }
      }
